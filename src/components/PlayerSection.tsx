...
export default function PlayerSection({ 
  ...
  return (
    <div className={`bg-black ${isCurrentUser ? 'bg-opacity-50' : 'bg-opacity-40'} rounded-xl p-4 backdrop-blur-sm ${
      isCurrentUser ? 'border border-poker-gold border-opacity-30' : ''
    }`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center space-x-3">
          <div className={`w-10 h-10 ${isCurrentUser ? 'bg-poker-gold' : 'bg-gray-400'} rounded-full flex items-center justify-center`}>
        ...
        <div className="text-right">
          <span className={`font-bold text-sm ${
            ...
            isCurrentUser ? 'text-green-400' : 'text-poker-gold'
          }`}>
      ...
      {handStrength && isCurrentUser && (
        <div className="text-center">
          <div className="bg-gradient-to-r from-yellow-500 to-poker-gold text-black px-3 py-1 rounded-full text-sm font-semibold inline-block">
            {handStrength}
          </div>
        </div>
      )}
