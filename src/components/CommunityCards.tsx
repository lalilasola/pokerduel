  ...
  return (
    <div className="text-center">
      ...
      <div className="bg-black bg-opacity-30 rounded-lg px-4 py-2 mb-4">
        <span className="text-poker-gold font-semibold">Pot: ${pot.toLocaleString()}</span>
      </div>
      ...
      <div className="flex justify-center space-x-3 mb-4">
        {[0, 1, 2, 3, 4].map((index) => (
          <div key={index} className="relative">
            {cards[index] ? (
              <div className="w-16 h-20 bg-white border-2 border-gray-300 rounded-lg shadow-lg flex flex-col items-center justify-center transform hover:scale-105 transition-transform animate-card-flip">
      ...
      <div className="bg-black bg-opacity-40 backdrop-blur-sm rounded-lg px-4 py-2 inline-block">
        <span className="text-poker-gold font-semibold text-sm">{getPhaseDisplay(phase)}</span>
      </div>
    </div>
  );
}
