...
export default function WinnerModal({ winner, onPlayAgain }: WinnerModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gradient-to-br from-poker-green to-green-700 rounded-2xl p-8 max-w-md w-full border-4 border-poker-gold animate-winner-glow">
        <div className="text-center">
          <div className="w-20 h-20 bg-poker-gold rounded-full flex items-center justify-center mx-auto mb-4">
          ...
          <div className="bg-black bg-opacity-30 rounded-lg p-4 mb-6">
            <p className={`font-bold text-xl ${isPlayerWinner || isTie ? 'text-poker-gold' : 'text-red-400'}`}>
              {isPlayerWinner || isTie ? '+' : '-'}${winner.winnings.toLocaleString()}
            </p>
          </div>
          <button
            onClick={onPlayAgain}
            className="bg-poker-gold text-black px-6 py-3 rounded-lg font-bold hover:bg-yellow-400 transition-colors"
          >
            Play Again
          </button>
        </div>
